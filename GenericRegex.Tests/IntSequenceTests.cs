...
namespace GenericRegex.Tests
{
    public class IntSequenceTests
    {
        ...
        {
            var seq = new int[] { 5, 3, 0, 10, 9, 1, 1, 1, 0, 9, 3, 7, 1, -2, 9, 0, 6, 7, 8, 1, 1, 5, 0, 9, 8, 7, 2, 0, 10, 10, 10, 0, 5 };
            var pattern = Seq(0, OneOrMany(Pred(n => n > 5)), Or(0, OneOrMany(1)));

            var match = pattern.FindMatchesIn(seq).ToList();
        ...
        {
            var seq = new int[] { 6, 5, 3, 6, 7, 4, 2, 3, 4, 5, 8 };
